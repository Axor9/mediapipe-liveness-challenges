...
import { boundingBox } from '@/utils/functions'
...
export default class FaceDetector {
    ...
    constructor(options?: Options) {
        if (options) this.faceMesh.setOptions(options)
    ...
    async getLandmarks(results: Results) {
        ...
        if (results.multiFaceLandmarks[0] != undefined) {
            ...
            const nose: Rect = {
                x: faceLandmarks[indexNose[0]].x * width,
                y: faceLandmarks[indexNose[0]].y * height,
                width: 6,
                height: 6,
            }
